...
if($_POST){
    if(!isset($_POST['order'])){
        update_option('_hb_woo_line_st',$result ='');
    }else{
        $verifyHtml = [];
        ...
        update_option('_hb_woo_line_st_2',$verifyHtml);
    }
}
?>
<div id="app">
    <div class="stbox">
        <h2>&ensp;<?php esc_html_e("Order Event","hb-line-tools-for-woocommerce");?></h2>
    </div>
    ...
    <div>
        ...
        <div class="stbox" v-for="(item,index) in orderEvent" v-bind:key="item.id">
            <div class="order-status" :id="item.id">
                ...
                <p>
                    <?php esc_html_e("When the order is changed to","hb-line-tools-for-woocommerce");?>
                    <select :name=" 'order[' + item.id + '][Statusestype]' " v-model="item.Statusestype">

                        <template v-for="(Statuses, indexStatuses) in OrderAllStatuses" >
                            <option :value="indexStatuses" >{{Statuses}}</option>
                        </template>
                    </select>
                    <?php esc_html_e("send message","hb-line-tools-for-woocommerce");?>
                </p>
                <p>
...
<script>
    const app = Vue.createApp({
        ...
        created(){
            this.getOrderAllStatuses();
            this.getOrderEvent();

        },
        methods:{
            addEvent:function (){
                this.orderEvent.push({id:vm.hbId,type:'text',Statusestype:'wc-completed'});
                vm.hbId++
            },
            deleteEvent: function(index) {
                this.orderEvent.splice(index, 1);
            },
            getOrderAllStatuses:function (){
                var data = new FormData();
                data.append('action', 'get_AllStatuses_action');
                axios.post('admin-ajax.php', data)
                    .then(function (response) {
                       vm.OrderAllStatuses =  response.data
                    })
            },
            getOrderEvent:function (){
                //URLSearchParams
                var data = new FormData();
                data.append('action', 'get_Order_action');
                        ...
                        for (let i = 0; i < array.length; i++) {
                            if(array[i]['type']!='template'){
                                ...
                                vm.orderEvent.push({
                                    ...
                                    type:array[i]['type'],
                                    text:array[i]['text'],
                                    Statusestype:array[i]['Statusestype'],
                                });
